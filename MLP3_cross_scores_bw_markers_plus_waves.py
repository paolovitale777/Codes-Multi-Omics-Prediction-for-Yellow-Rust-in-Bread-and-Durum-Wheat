...
X_training = X_training.loc[valid_idx_2].values.astype(np.float32)
y_training = pheno.loc[valid_idx_2, training_col].values.astype(np.float32)

X_test = X_test.loc[valid_idx_2].values.astype(np.float32)
...
for hidden_sizes, lr, batch_size, dropout, wd, epochs, L1 in grid:
            ...
            for inner_train_idx, inner_val_idx in inner_cv.split(X_training):
                ...
                model = MLP(X_training.shape[1], hidden_sizes, dropout)
...
model = MLP(X_training.shape[1], best_params.hidden_sizes, best_params.dropout)
...
results_file = os.path.join(output_dir, "MLP3_PA_cross_scores_bw_markers_plus_waves.csv")
all_results_df.to_csv(results_file, index=False)
print("Results:", all_results_df)

loss_df = pd.DataFrame(loss_tracking)
loss_df.to_csv(os.path.join(output_dir, "MLP3_loss_per_epochr_cross_scores_bw_markers_plus_waves.csv"), index=False)