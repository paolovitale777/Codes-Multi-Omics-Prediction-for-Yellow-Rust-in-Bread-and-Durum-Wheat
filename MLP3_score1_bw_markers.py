...
X = markers.loc[valid_idx].values.astype(np.float32)
...
results_file = os.path.join(output_dir, "MLP3_PA_score1_bw_markers_2.csv")
all_results_df.to_csv(results_file, index=False)

loss_df = pd.DataFrame(loss_tracking)
loss_df.to_csv(os.path.join(output_dir, "MLP3_loss_per_epoch_score1_bw_markers_2.csv"), index=False)
