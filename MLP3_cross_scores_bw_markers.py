...
results_file = os.path.join(output_dir, "MLP3_PA_cross_scores_bw_markers.csv")
all_results_df.to_csv(results_file, index=False)
print("Results:", all_results_df)

loss_df = pd.DataFrame(loss_tracking)
loss_df.to_csv(os.path.join(output_dir, "MLP3_loss_per_epochr_cross_scores_bw_markers.csv"), index=False)