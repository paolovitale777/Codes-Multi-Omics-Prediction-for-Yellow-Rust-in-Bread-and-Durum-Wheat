...
X = X_combined.loc[valid_idx].values.astype(np.float32)
...
param_grid = {
    ...
    "dropout": [0.5, 0.8], # Added 0.8
    "weight_decay": [0.5, 1.0, 0.7], 
    "epochs": [200],
    "L1": [0.5, 1.0, 0.7]
}
...
results_file = os.path.join(output_dir, "MLP3_PA_score1_bw_markers_plus_waves.csv")
all_results_df.to_csv(results_file, index=False)

loss_df = pd.DataFrame(loss_tracking)
loss_df.to_csv(os.path.join(output_dir, "MLP3_loss_per_epoch_score1_bw_markers_plus_waves.csv"), index=False)
